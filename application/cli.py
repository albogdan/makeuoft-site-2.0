...
def register(flask_app):
    ...
    def users():
        ...
        with open(users_csv_path, 'r') as users_csv:
            for i in users_csv:
                j = i[:-1].split(',')
                record = User(first_name=j[0], last_name=j[1], email=j[2], id_provided = bool(random.getrandbits(1)))
...
#            for i in range(0, 5):#len(userSeeds)//Teams.max_members):
                code = ''.join(random.choice(string.ascii_uppercase) for i in range(5)) #5 char ID
